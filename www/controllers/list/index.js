...
.controller("ListMainController", function($scope, $rootScope, $ionicViewSwitcher, $ionicLoading, $document, $timeout, $ionicScrollDelegate, toolPanel, Common, Product, Tour){
		...
		(function(){
			...
			$scope.filterShow = function(){
				...
				toolPanel.onBeforeShow = function(){
					scope.switchTab = switchTab(toolPanel.container);
					scope.switchTab(1);
				};
...
.controller("ListFilterController", function($scope, Common, Product, Tour){
	$scope.filterPanel = filterTemplate;
	var scope = $scope;
	// 出发城市数据源
	Common.StartCity(function(data){
		scope.startCitySource = data;
